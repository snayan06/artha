  ...
  return (
    <div className="space-y-5 sm:space-y-7">
      <div className="flex items-end justify-between">
        <div>
          <p className="text-sm font-medium text-[#728078] tone-muted">Good morning, {profile.displayName}</p>
          <h1 className="font-display mt-1 text-2xl font-bold tracking-[-0.04em] sm:text-3xl">Your money, made clear.</h1>
        </div>
      ...
      <div className="grid gap-5 lg:grid-cols-[1.2fr_.8fr]">
        <Card className="overflow-hidden border-0 bg-moss-900 p-5 text-white dark:bg-night-raised sm:p-7">
          <div className="flex items-center justify-between">
            <div className="flex items-center gap-2 text-sm text-moss-200"><WalletCards className="h-4 w-4" /> Available across accounts</div>
            <button onClick={() => setShowBalance((value) => !value)} className="grid h-11 w-11 place-items-center rounded-full bg-white/10 text-moss-100" aria-label={showBalance ? 'Hide balance' : 'Show balance'}>
              {showBalance ? <Eye className="h-4 w-4" /> : <EyeOff className="h-4 w-4" />}
            </button>
          </div>
          <p className="font-display mt-5 text-[38px] font-semibold leading-none tracking-[-0.05em] sm:text-5xl">
          ...
          <div className="mt-7 grid grid-cols-2 gap-3">
            <div className="rounded-2xl bg-white/[0.07] p-3.5">
              ...
              <p className="mt-1.5 text-lg font-semibold">{formatMoney(dashboard.incomePaise)}</p>
            </div>
            <div className="rounded-2xl bg-white/[0.07] p-3.5">
              <p className="text-xs text-moss-200">Spending tracked</p>
              <p className="mt-1.5 text-lg font-semibold">{formatMoney(dashboard.spendPaise)}</p>
            </div>
          </div>
        </Card>
        ...
        <Card className="flex flex-col justify-between p-5 sm:p-6">
          <div className="flex items-start justify-between">
            <div>
              ...
              <p className="font-display mt-2 text-3xl font-bold tracking-[-0.04em] text-moss-900">{formatMoney(dashboard.sharedBalancePaise)}</p>
              <p className="mt-1 text-xs text-[#7c8781] tone-muted">{sharedLabel}</p>
            </div>
            <div className="grid h-11 w-11 place-items-center rounded-2xl bg-moss-100 text-moss-800"><UsersRound className="h-5 w-5" /></div>
          </div>
          <AppLink to="/shared" className="mt-6 flex items-center justify-between rounded-2xl bg-moss-50 px-4 py-3 text-sm font-semibold text-moss-800 transition hover:bg-moss-100">
            Review shared expenses <ArrowRight className="h-4 w-4" />
          </AppLink>
        </Card>
      </div>
      ...
      <Card className="p-4 sm:p-5">
        <form onSubmit={startCapture}>
          <div className="mb-3 flex items-center gap-2 text-sm font-semibold"><Sparkles className="h-4 w-4 text-moss-600" /> What happened?</div>
          <div className="flex flex-col gap-3 sm:flex-row">
            <label className="sr-only" htmlFor="home-capture">Describe a transaction</label>
            <input id="home-capture" value={capture} onChange={(event) => setCapture(event.target.value)} placeholder={profile.members[0] ? `Paid 850 for dinner with ${profile.members[0].name}` : 'Paid 850 for dinner yesterday'} className="min-h-12 min-w-0 flex-1 rounded-2xl border border-line bg-[#fafbf9] px-4 text-[15px] outline-none transition placeholder:text-[#9ca69f] tone-subtle focus:border-moss-400 focus:ring-4 focus:ring-moss-100 dark:bg-night-input" />
            <Button type="submit" disabled={!capture.trim()} className="sm:px-6">Make draft <ArrowRight className="h-4 w-4" /></Button>
          </div>
