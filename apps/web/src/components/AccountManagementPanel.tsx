...
function AccountEditor({ account, onSave, onCancel }: { account?: ManagedAccount; onSave: (input: import('../types').AccountSetupInput) => Promise<void>; onCancel: () => void }) {
  ...
  return <form onSubmit={(event) => { event.preventDefault(); setBusy(true); void onSave({ name, kind, opening_balance_paise: account?.openingBalancePaise ?? (kind === 'credit_card' ? -Math.abs(rupeesToPaise(Number(balance))) : rupeesToPaise(Number(balance))), credit_limit_paise: kind === 'credit_card' && limit ? rupeesToPaise(Number(limit)) : null, statement_day: kind === 'credit_card' && statement ? Number(statement) : null, payment_due_day: kind === 'credit_card' && due ? Number(due) : null }).finally(() => setBusy(false)) }} className="mt-4 grid gap-3 rounded-2xl bg-moss-50 p-4 dark:bg-night-raised sm:grid-cols-2">
