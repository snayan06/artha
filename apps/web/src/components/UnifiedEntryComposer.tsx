    ...
    <form onSubmit={submitForm}>
      ...
      <div className={variant === 'full' ? 'space-y-3' : 'flex flex-col gap-3 sm:flex-row'}>
        ...
        <div className={variant === 'full' ? 'grid gap-2 sm:flex' : 'contents'}>
          ...
          {secondaryAction && <Button type="button" variant="secondary" className={variant === 'full' ? 'w-full sm:w-auto' : undefined} onClick={runSecondaryAction}>{secondaryAction.label}</Button>}
        </div>
      </div>
      {variant === 'full' && <p className="mt-2 text-xs text-[#7b8781] tone-muted">Enter to continue · Shift+Enter for a new line.</p>}
      ...
      {(recovery === 'clarify' || recovery === 'failure') && (
        <Card className="mt-4 p-4 shadow-none" role={recovery === 'failure' ? 'alert' : 'status'} aria-live={recovery === 'failure' ? 'assertive' : 'polite'}>
