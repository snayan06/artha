...
function renderComposer({
  ...
  function TestComposer() {
    ...
    return (
      <UnifiedEntryComposer
        ...
        secondaryAction={onSecondaryAction ? { label: 'Enter details manually', onClick: onSecondaryAction } : undefined}
      />
    )
  }
  const view = render(
  ...
  return { onCapture, onAskLedger, ...view }
}
...
describe('UnifiedEntryComposer', () => {
  ...
  it('ignores a delayed route after the composer unmounts', async () => {
    let resolveRoute!: (value: { intent: 'ask_ledger' }) => void
    vi.mocked(routeIntent).mockReturnValue(new Promise((resolve) => { resolveRoute = resolve }))
    const user = userEvent.setup()
    const { onAskLedger, unmount } = renderComposer({ initialValue: 'Show my balance' })

    await user.click(screen.getByRole('button', { name: 'Continue' }))
    unmount()
    resolveRoute({ intent: 'ask_ledger' })
    await Promise.resolve()

    expect(onAskLedger).not.toHaveBeenCalled()
  })

  it('cancels a delayed route before running the manual-entry action', async () => {
    let resolveRoute!: (value: { intent: 'capture_transaction' }) => void
    vi.mocked(routeIntent).mockReturnValue(new Promise((resolve) => { resolveRoute = resolve }))
    const user = userEvent.setup()
    const manual = vi.fn()
    const { onCapture } = renderComposer({ initialValue: 'Paid 500', onSecondaryAction: manual })

    await user.click(screen.getByRole('button', { name: 'Continue' }))
    await user.click(screen.getByRole('button', { name: 'Enter details manually' }))
    resolveRoute({ intent: 'capture_transaction' })
    await Promise.resolve()
