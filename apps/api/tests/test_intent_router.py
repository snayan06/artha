...
async def test_gemini_router_returns_only_the_validated_intent() -> None:
    ...
    assert "settlement" in str(call["system_instruction"])
    assert "unsupported" in str(call["system_instruction"])
...
@pytest.mark.asyncio
async def test_router_fails_closed_for_interactions_transport_errors() -> None:
    assistant = LocalFinancialAssistant(
        AssistantSettings(
            provider=LlmProvider.GEMINI,
            gemini_api_key="gemini-test-key",
