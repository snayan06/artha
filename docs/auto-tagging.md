...
The hosted default is Qwen3.6-27B through Groq. Local/private use can select Qwen3
4B through Ollama. Both providers implement the same internal interface and
strict response schema. When a provider is missing, rate-limited, or unavailable,
Hisab falls back to manual category selection without blocking transaction entry.
