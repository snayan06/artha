...
| --- | --- | --- |
| Unified intent entry | Candidate in separate branch | One composer routes transactions to review and questions directly to Ask Artha; 48 keyless cases pass. PR review, CI, deployment and live QA remain |
...
| V1 capture hardening | Deployed and accepted | PR #21 merged as `c4ae0dc`; manual Expense/Income/Transfer recovery and grounded category/account context passed final-domain fictional QA plus 170 web + 223 API + 50/30/24 AI contracts |
...
- [x] Add 48 fictional router cases with near-neighbours, ambiguity, Hinglish,
  unsupported actions and prompt injection; false capture is a safety failure.
- [ ] Publish the unified-entry PR, pass CI/deployment, then accept both routed
