...
Date: 7 August 2026
Status: provider-neutral LLM boundary deployed; current production provider is Gemini

## Current production usage

| Feature | LLM job | Code-owned boundary | Failure behavior | May write? |
| --- | --- | --- | --- | --- |
...
The current production configuration is `gemini-3.5-flash-lite`, called
server-side through Google's official SDK. The surrounding schemas, validation
and authority boundaries are provider-neutral. Explicit Ollama selection is
available only for local developer use; it is not a production provider or a
production fallback.
...
The LLM may interpret phrases such as `25k`, `three days ago`, account aliases
...
Production Quick Add and production tag suggestion call the configured LLM
without loading `merchant_rules`. For the standalone tag endpoint, FastAPI—not
the caller—loads up to 200 active, direction-eligible categories from the
authenticated household and supplies that allow-list to the model. The V1 web
app does not call this endpoint; its Quick Add category is separate capture
output. Merchant-rule-first
...
3. The LLM selects an intent and copies its approved narrative and widget array.
